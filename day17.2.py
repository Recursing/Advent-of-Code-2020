...
def neighbours(x: int, y: int, z: int, w: int) -> Iterator[tuple[int, int, int, int]]:
        ...
        for y in range(-max_x, max_x):
            for z in range(-max_z, max_z + 1):
                for w in range(-max_z, max_z + 1):
                    neighs = neighbours(x, y, z, w)
                    n_active = sum(old_grid[c] for c in neighs)
                    new_grid[(x, y, z, w)] = n_active == 3 or (
                        old_grid[(x, y, z, w)] and n_active == 2
                    )
    return new_grid
